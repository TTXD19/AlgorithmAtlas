...
const PROVIDERS: { id: Provider; labelKey: "withGitHub" | "withGoogle"; icon: React.ReactNode }[] = [
  {
    id: "github",
    labelKey: "withGitHub",
    icon: (
  ...
  return (
    <div ref={wrapRef} className="relative">
      <button
        ...
        title={user ? displayName(user) : t.auth.signIn}
      >
        {user ? displayName(user).charAt(0).toUpperCase() : t.auth.signIn}
      </button>
          ...
          {user ? (
            ...
            <>
              ...
              {PROVIDERS.map((p) => (
                ...
                >
                  {p.icon}
                  {busy === p.id ? t.auth.signingIn : t.auth[p.labelKey]}
                </button>
              ))}
            </>
          )}
