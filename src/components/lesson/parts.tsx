...
export const SECTIONS = [
  ...
  { id: "steps", title: "演算法步驟" },
  { id: "demo", title: "互動示範" },
  { id: "code", title: "程式碼" },
  { id: "problems", title: "練習題" },
] as const;
...
export function Applications({ items, cue }: { items: Scenario[]; cue: string }) {
  return (
    <>
      <div className="grid max-w-[72ch] grid-cols-1 gap-2.5">
        {items.map((it) => (
          <div key={it.title} className="rounded-[10px] border border-line bg-surface px-4 py-3">
            ...
            <p className="m-0 text-[14px]"><span className="mr-1.5 font-semibold text-accent">為什麼用它</span>{it.why}</p>
          </div>
        ))}
      </div>
      <p className="mt-3.5 max-w-[72ch] rounded-lg bg-accent-soft px-4 py-2.5 text-[14px] text-ink">
        <span className="mr-1.5 font-semibold">看到這些關鍵字就想到它：</span>{cue}
      </p>
    </>
  );
}
