...
import { DEMO_GRAPH, DEMO_CONFIGS, type DemoAlgo } from "@/lib/graph-demo";
...
export function GraphDemo({ algo }: { algo: DemoAlgo }) {
  const config = DEMO_CONFIGS[algo];
    ...
    <div className="@container overflow-hidden rounded-xl border border-line bg-surface">
      <div className="flex flex-wrap items-center gap-2.5 border-b border-line bg-surface-2 px-3.5 py-2.5 text-[13px] text-ink-2">
        <div className="flex gap-1.5">
          <button type="button" className={BTN} onClick={() => setI((x) => Math.max(0, x - 1))} disabled={i === 0}>上一步</button>
          <button type="button" className={BTN_PRIMARY} onClick={() => setI((x) => Math.min(steps.length - 1, x + 1))} disabled={i === steps.length - 1}>下一步</button>
          <button type="button" className={BTN} onClick={() => setI(0)}>重設</button>
        </div>
