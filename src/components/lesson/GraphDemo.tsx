...
const TEXT = demoText(
  ...
  {
    en: {
      graphLabel: "Traversal demo graph",
    ...
    <div className="@container overflow-hidden rounded-xl border border-line bg-surface">
      <div className="flex flex-wrap items-center gap-2.5 border-b border-line bg-surface-2 px-3.5 py-2.5 text-[13px] text-ink-2">
        <div className="flex gap-1.5">
          <button type="button" className={BTN} onClick={() => setI((x) => Math.max(0, x - 1))} disabled={i === 0}>{ui.demo.prev}</button>
          <button type="button" className={BTN_PRIMARY} onClick={() => setI((x) => Math.min(steps.length - 1, x + 1))} disabled={i === steps.length - 1}>{ui.demo.next}</button>
          <button type="button" className={BTN} onClick={() => setI(0)}>{ui.demo.reset}</button>
        </div>
      ...
      <div className="grid grid-cols-1 @[640px]:grid-cols-[minmax(0,1fr)_220px]">
        <svg viewBox="0 0 630 260" role="img" aria-label={t.graphLabel} className="block h-auto w-full">
          ...
          <div className="flex-1">
            ...
            <div className="min-h-5 font-mono text-[13px] tracking-[0.04em]">
              {s.order.length ? (
                ...
                <span className="font-sans text-ink-3">{t.notStarted}</span>
              )}
            </div>
          </div>
        </div>
