...
const TEXT = demoText(
  {
    intro: "堆疊存「還沒找到更高溫度的日子」的索引。從左到右每天處理一次。",
    pop: (i: number, hot: number, j: number, cool: number, wait: number) =>
      `第 ${i} 天 ${hot}° 比堆疊頂端第 ${j} 天的 ${cool}° 高：第 ${j} 天的答案出來了，等 ${wait} 天。彈出。`,
    pushAfterPops: (i: number) => `沒有更矮的了，把第 ${i} 天推入，它現在也在等更高的溫度。`,
    topNote: (j: number, temp: number) => `（頂端第 ${j} 天 ${temp}°）`,
    pushKeep: (i: number, temp: number, note: string) =>
      `第 ${i} 天 ${temp}° 沒有比頂端高${note}，直接推入。堆疊由底到頂溫度遞減。`,
    finished: "走完了。堆疊裡剩下的日子之後都沒有更高溫，答案 0。每個索引只推入一次、彈出一次，O(n)。",

    tempTitle: "溫度 / 答案（要等幾天）",
    stackTitle: "堆疊（頂 → 底）",
  },
  {
    en: {
      intro: "The stack holds the indices of the days that are still waiting for a warmer one. Work through the days from left to right, one at a time.",
      pop: (i: number, hot: number, j: number, cool: number, wait: number) =>
        `Day ${i} at ${hot}° is warmer than day ${j} at ${cool}° on top of the stack, so day ${j} finally has its answer: it waits ${wait} day${wait === 1 ? "" : "s"}. Pop it.`,
      pushAfterPops: (i: number) => `Nothing cooler is left on top, so push day ${i}; it is now the one waiting for a warmer day.`,
      topNote: (j: number, temp: number) => ` (the top is day ${j} at ${temp}°)`,
      pushKeep: (i: number, temp: number, note: string) =>
        `Day ${i} at ${temp}° is not warmer than the top${note}, so simply push it. The stack decreases in temperature from bottom to top.`,
      finished: "The scan is over. The days still on the stack never meet a warmer day, so their answer is 0. Every index is pushed once and popped once, which makes the whole thing O(n).",

      tempTitle: "Temperature / answer (days to wait)",
...
function buildSteps(t: Txt): Step[] {
  ...
  steps.push({ desc: t.intro, i: -1, stack: [], ans: [...ans] });
  for (let i = 0; i < T.length; i++) {
    ...
    while (st.length && T[st[st.length - 1]] < T[i]) {
      ...
      ans[j] = i - j;
      popped++;
      steps.push({ desc: t.pop(i, T[i], j, T[j], i - j), i, stack: [...st], ans: [...ans], popped: j });
    }
    st.push(i);
    steps.push({ desc: popped ? t.pushAfterPops(i) : t.pushKeep(i, T[i], st.length > 1 ? t.topNote(st[st.length - 2], T[st[st.length - 2]]) : ""), i, stack: [...st], ans: [...ans] });
  }
  ans.forEach((a, j) => { if (a === null) ans[j] = 0; });
  steps.push({ desc: t.finished, i: T.length, stack: [...st], ans: [...ans] });
  return steps;
}
    ...
    <div className="overflow-hidden rounded-xl border border-line bg-surface">
      ...
      <div className="grid grid-cols-1 gap-4 p-3.5 md:grid-cols-[minmax(0,1fr)_150px]">
        <div className="overflow-x-auto">
          <div className="eyebrow mb-1.5">{t.tempTitle}</div>
