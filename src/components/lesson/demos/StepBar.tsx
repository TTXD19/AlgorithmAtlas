  ...
  return (
    <div className="flex flex-wrap items-center gap-2.5 border-b border-line bg-surface-2 px-3.5 py-2.5 text-[13px] text-ink-2">
      <div className="flex gap-1.5">
        <button type="button" className={BTN_PLAIN} onClick={() => setK((x) => Math.max(0, x - 1))} disabled={k === 0}>上一步</button>
        <button type="button" className={BTN_PRIMARY} onClick={() => setK((x) => Math.min(total - 1, x + 1))} disabled={k >= total - 1}>下一步</button>
        <button type="button" className={BTN_PLAIN} onClick={() => setK(() => 0)}>重設</button>
      </div>
      {left}
...
export function StepFooter({ k, total, children }: { k: number; total: number; children: ReactNode }) {
  return (
    <div className="flex items-center gap-3 border-t border-line px-3.5 py-2.5 text-[13.5px]">
      <span className="shrink-0 font-mono text-[12px] text-ink-3">步驟 {k}/{total - 1}</span>
...
export function Cells({ items, tone = () => "", empty = "空", w = "w-9" }: { items: (string | number)[]; tone?: (i: number) => string; empty?: string; w?: string }) {
  return (
    <div className="flex flex-wrap gap-1">
      ...
      {items.length === 0 && <span className="rounded-md border border-dashed border-line-strong px-2 text-[12px] leading-8 text-ink-3">{empty}</span>}
    </div>
  );
}
