...
const TEXT = demoText(
  {
    ...
    shuffle: "換一組數字",
    original: "原陣列 a",
    prefixRow: "前綴和 P（P[i] = a[0] + … + a[i-1]）",
    intro: "P[0] = 0 是起點。按「建表下一步」，每一步只做一次加法：P[i+1] = P[i] + a[i]。",
    step: (i: number, prev: number, a: number, now: number, count: number) =>
      `P[${i}] = P[${i - 1}] + a[${i - 1}] = ${prev} + ${a} = ${now}。到目前為止做了 ${count} 次加法。`,
    queryL: "查詢區間 l =",
    oneSubtraction: (k: number) => `一次減法。暴力做法要加 ${k} 次。`,
  },
  {
    en: {
      buildStep: "Build one step",
      buildAll: "Build all",
      reset: "Reset",
      shuffle: "New numbers",
      original: "Original array a",
      prefixRow: "Prefix sums P (P[i] = a[0] + … + a[i-1])",
      intro: 'P[0] = 0 is the starting point. Press "Build one step" — each step is a single addition: P[i+1] = P[i] + a[i].',
      step: (i: number, prev: number, a: number, now: number, count: number) =>
        `P[${i}] = P[${i - 1}] + a[${i - 1}] = ${prev} + ${a} = ${now}. That is ${count} addition${count === 1 ? "" : "s"} so far.`,
      queryL: "Query range l =",
      oneSubtraction: (k: number) => `One subtraction. Brute force would add ${k} numbers.`,
    },
  },
);
  ...
  return (
    <div className="overflow-hidden rounded-xl border border-line bg-surface">
      <div className="flex flex-wrap items-center gap-1.5 border-b border-line bg-surface-2 px-3.5 py-2.5 text-[13px] text-ink-2">
        <button type="button" className={`${BTN} border-accent bg-accent text-accent-ink hover:brightness-110`} onClick={() => setBuilt((b) => Math.min(n, b + 1))} disabled={ready}>{t.buildStep}</button>
        <button type="button" className={`${BTN} border-line bg-surface hover:bg-surface-2`} onClick={() => setBuilt(n)} disabled={ready}>{t.buildAll}</button>
        <button type="button" className={`${BTN} border-line bg-surface hover:bg-surface-2`} onClick={() => reset(START)}>{t.reset}</button>
        <button type="button" className={`${BTN} ml-auto border-line bg-surface hover:bg-surface-2`} onClick={() => reset(randomArr())}>{t.shuffle}</button>
      </div>

      <div className="overflow-x-auto px-3.5 pt-3.5">
        <div className="eyebrow mb-1.5">{t.original}</div>
        <Row cells={arr.map((v, i) => ({ v, hi: ready && i >= l && i <= r ? "range" : built === i + 1 ? "cur" : "" }))} offset={0} />

        <div className="eyebrow mt-3.5 mb-1.5">{t.prefixRow}</div>
        <Row
      ...
      <div className="border-t border-line px-3.5 py-2.5 text-[13.5px]">
        {!ready ? (
          built === 0
            ? t.intro
            : t.step(built, prefix[built - 1], arr[built - 1], prefix[built], built)
        ) : (
          <div className="flex flex-col gap-2.5">
            ...
            <div>
              ...
              <span className="ml-2 text-ink-3">{t.oneSubtraction(r - l + 1)}</span>
            </div>
          </div>
        )}
      </div>
